...
{       
     ...
     * @var string
    */
    protected $app_name=Gc::APP_FLAG; 
    /**
     * 是否同步进行远程对象通信操作。同步会对当前运行的程序造成阻塞
     * @var bool
     */
    public $IsSync=Gc::IS_SYNC;    
    /**
     ...
     * @var bool
     */
    public $Is_Url_Rewrite=Gc::IS_URL_REWRITE;
    /**
     * 响应数据类型
     * @var enum 
     */
    public $response_type=Gc::RESPONSE_TYPE;
    ...
    public function sendRequest($method=EnumHttpMethod::POST,$response_type=EnumResponseType::XML)
    {
        ...
        if ($Is_Url_Rewrite){
            return Manager_Communication::newInstance()->currentComm()->sendRequest(
                   Gc::$server_addr.$this->classname(),$data,$method,$response_type);
        }else{            
            return Manager_Communication::newInstance()->currentComm()->sendRequest(
                   Gc::$server_addr.Gc::$index_file.$this->classname(),$data,$method,$response_type);
        }        
    }
